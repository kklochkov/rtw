...
struct Color
{
  ...
  constexpr Color(const T r, const T g, const T b, const T a = 0xFF)
  ...
  constexpr void set_r(const std::uint8_t r)
  {
    rgba = (rgba & 0x00'FF'FF'FFU) | (static_cast<std::uint32_t>(r) << 24U);
  }
  constexpr std::uint8_t r() const { return (rgba >> 24U) & 0xFFU; }
  ...
  constexpr void set_g(const std::uint8_t g)
  {
    rgba = (rgba & 0xFF'00'FF'FFU) | (static_cast<std::uint32_t>(g) << 16U);
  }
  constexpr std::uint8_t g() const { return (rgba >> 16U) & 0xFFU; }
  ...
  constexpr void set_b(const std::uint8_t b) { rgba = (rgba & 0xFF'FF'00'FFU) | (static_cast<std::uint32_t>(b) << 8U); }
  constexpr std::uint8_t b() const { return (rgba >> 8U) & 0xFFU; }
  ...
  constexpr void set_a(const std::uint8_t a) { rgba = (rgba & 0xFF'FF'FF'00U) | a; }
  constexpr std::uint8_t a() const { return rgba & 0xFFU; }
