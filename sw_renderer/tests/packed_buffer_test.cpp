...
using PackedBuffer = rtw::stl::PackedBuffer<Struct>;

} // namespace

TEST(PackedBufferTest, constructor)
{
  ...
  EXPECT_DEATH(PackedBuffer{0U}, ".*");
}

TEST(PackedBufferTest, push_back)
{
  PackedBuffer buffer{10U};
  EXPECT_EQ(buffer.size(), 0U);
  EXPECT_EQ(buffer.capacity(), 10U);
...
TEST(PackedBufferTest, emplace_back)
{
  PackedBuffer buffer{10U};
  EXPECT_EQ(buffer.size(), 0U);
  EXPECT_EQ(buffer.capacity(), 10U);
