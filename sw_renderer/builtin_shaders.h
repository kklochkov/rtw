...
{
constexpr inline std::uint32_t POSITION{0U}; ///< Object-space position (vec3/vec4; w defaults to 1).
constexpr inline std::uint32_t NORMAL{1U};   ///< Object-space normal (vec3).
constexpr inline std::uint32_t UV{2U};       ///< Texture coordinate (vec2).
constexpr inline std::uint32_t COLOR{3U};    ///< Vertex colour (vec4 rgba).
} // namespace attribute_location
...
{
public:
  constexpr static std::uint32_t COLOR_VARYING{0U}; ///< Varying slot for vertex colour (vec4 rgba).
...
{
public:
  constexpr static std::uint32_t UV_VARYING{0U}; ///< Varying slot for texture coordinate (vec2).
...
/// Per-fragment Lambert diffuse lighting with a single directional light.
///
/// The normal is transformed to world space as a direction (w = 0) by `normal_matrix`,
/// interpolated across the triangle, then re-normalized and shaded per fragment.
class LitShader : public IShaderProgram
{
public:
  constexpr static std::uint32_t NORMAL_VARYING{0U}; ///< Varying slot carrying the interpolated world normal.
  ...
  FragmentShaderOutput fragment(const DynamicVaryings& input, const FragmentContext& /*context*/) const override
  {
    ...
    // Right-handed convention: the light points along its travel direction, so the surface faces the
    // light when their dot product is negative (matches the legacy fixed-function lighting).
