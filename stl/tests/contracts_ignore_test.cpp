...
TEST(ContractsIgnoreTest, current_semantic)
{
  EXPECT_EQ(rtw::stl::contracts::current_semantic(), rtw::stl::contracts::EvaluationSemantic::IGNORE);
}

TEST(ContractsIgnoreTest, simple)
{
  EXPECT_NO_FATAL_FAILURE(RTW_PRE(true, "This precondition should pass"));
  RTW_PRE(false);
  RTW_PRE(false, "This precondition should fail");

  EXPECT_NO_FATAL_FAILURE(RTW_ASSERT(true, "This precondition should pass"));
  RTW_ASSERT(false);
  RTW_ASSERT(false, "This precondition should fail");

  EXPECT_NO_FATAL_FAILURE(RTW_POST(true, "This precondition should pass"));
  RTW_POST(false);
  RTW_POST(false, "This precondition should fail");
...
TEST(ContractsIgnoreTest, function_with_contracts)
{
  EXPECT_NO_FATAL_FAILURE(function_with_contracts(10));
  EXPECT_NO_FATAL_FAILURE(function_with_contracts(-10));
  EXPECT_NO_FATAL_FAILURE(function_with_contracts(11));
  EXPECT_NO_FATAL_FAILURE(function_with_contracts(50));
}

TEST(ContractsIgnoreTest, function_throws_exception)
{
  EXPECT_NO_FATAL_FAILURE(RTW_PRE(function_throws_exception(10) < 100, "Value must be less than 100"));
  EXPECT_NO_FATAL_FAILURE(RTW_PRE(function_throws_exception(110) < 100, "Value must be less than 100"));
}
