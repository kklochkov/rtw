...
{
  EXPECT_EQ(rtw::stl::contracts::current_semantic(), rtw::stl::contracts::EvaluationSemantic::ENFORCE);
}
...
TEST(ContractsEnforceTest, function_with_contracts)
{
  EXPECT_NO_FATAL_FAILURE(function_with_contracts(10));
  EXPECT_DEATH(function_with_contracts(-10), ::testing::StartsWith("Contract violation detected:\n"));
  EXPECT_DEATH(function_with_contracts(11), ::testing::StartsWith("Contract violation detected:\n"));
  EXPECT_DEATH(function_with_contracts(50), ::testing::StartsWith("Contract violation detected:\n"));
}

TEST(ContractsEnforceTest, function_throws_exception)
{
  EXPECT_NO_FATAL_FAILURE(RTW_PRE(function_throws_exception(10) < 100, "Value must be less than 100"));
  EXPECT_DEATH(RTW_PRE(function_throws_exception(110) < 100, "Value must be less than 100"),
               ::testing::StartsWith("Contract violation detected:\n"));
}
