...
using StaticPriorityQueue = rtw::stl::StaticPriorityQueue<Struct>;

} // namespace

TEST(PriorityQueueTest, constructor)
{
  StaticPriorityQueue queue{10U};
  ...
  EXPECT_DEATH(StaticPriorityQueue{0U}, ".*");
}

TEST(PriorityQueueTest, push_pop)
{
  StaticPriorityQueue queue{10U};
  EXPECT_EQ(queue.size(), 0U);
  EXPECT_EQ(queue.capacity(), 10U);
...
TEST(PriorityQueueTest, emplace_pop)
{
  StaticPriorityQueue queue{10U};
  EXPECT_EQ(queue.size(), 0U);
  EXPECT_EQ(queue.capacity(), 10U);
...
TEST(PriorityQueueTest, max_queue_additional_tests)
{
  rtw::stl::StaticPriorityQueue<std::int32_t> queue{10U};
  {
    queue.push(42);
...
TEST(PriorityQueueTest, min_queue_additional_tests)
{
  rtw::stl::StaticPriorityQueue<std::int32_t, std::greater<std::int32_t>> queue{10U};
  {
    queue.push(42);
    EXPECT_EQ(queue.top(), 42);
  }
