...
/// @brief A fixed-capacity, null-terminated string with value semantics.
///
/// GenericStaticString provides a std::string-like interface over a fixed-capacity character buffer.
/// The string is always null-terminated. Supports concatenation, comparison (via StringView), find,
/// substr, and iteration. CRTP-derived classes supply the actual storage (heap or inplace).
///
/// @tparam DerivedT CRTP derived class providing the backing buffer.
///
/// Complexity:
///   - push_back / pop_back: O(1)
///   - operator+=: O(n) where n = appended length
...
public:
  ...
  constexpr std::int32_t compare(const StringView sv) const noexcept { return StringView{data(), size()}.compare(sv); }
  ...
  // Comparison: GenericStaticString vs GenericStaticString
  ...
  // Comparison: GenericStaticString vs StringView (also covers const_pointer via StringView's implicit ctor)
  friend constexpr bool operator==(const GenericStaticString& lhs, const StringView rhs) noexcept
  {
    return lhs.compare(rhs) == 0;
  }
  friend constexpr bool operator!=(const GenericStaticString& lhs, const StringView rhs) noexcept
  ...
  // Comparison: StringView vs GenericStaticString (reversed)
  friend constexpr bool operator==(const StringView lhs, const GenericStaticString& rhs) noexcept
  {
...
/// @brief A fixed-capacity, inplace (stack-allocated) null-terminated string.
///
/// InplaceString stores its character buffer directly within the object using std::array.
/// It is trivially copyable and constexpr-friendly — no heap allocation is performed.
///
/// @tparam CAPACITY Maximum number of characters (excluding the null terminator).
